...
# 基本用法与原理
  ...
  Request定义了一系列匹配规则， 
     ...
     匹配规则也可以是对进入的请求消息的消息体的匹配，当前支持的消息体的格式是Json格式。用Json Pointer（rfc6901），我们可以很直接的定位到Json消息中某一个特定的值（注），
  ...
  2. 应答的消息体payload，其可以是一段Json数据，也可以是一段其它文本数据。如payload处的注解所言，此处填的payload可以带有变量占位符，每一个占位符对应后续一个argument
     ...
     从进入消息的Json消息体获得，用Json pointer来指定Json消息体中某一个特定的值（注）
...
# maock的吞吐量有多大：
  ...
  对于下面一个例子：
    ...
    对于进入的请求消息，匹配进入的:path header，并匹配user-Agent header，并匹配Json消息体中的某一个值
  
    对于生成的相应，包含三个可变header，共四个变量
